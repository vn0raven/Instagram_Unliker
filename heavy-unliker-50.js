/**
 * Instagram Like Remover - "Universal 50" 🛡️
 * 1. Batch Size: 50 (Safe & Stable).
 * 2. Scroller: Universal Brute-Force (Fixes scrolling issues).
 * 3. Crash Recovery: Auto-recovers if spinner gets stuck.
 */

;(async function () {
  // --- SETTINGS ---
  const BATCH_SIZE = 50
  const SAFETY_WAIT_MIN = 10000 
  ...
  console.log('Universal 50 Script started...')
  ...
  while (true) {
    try {
      ...
      for (const cb of checkboxes) {
        ...
        simulateClick(cb)
        count++
        await randomDelay(150, 250) // Relaxed clicks
      }
